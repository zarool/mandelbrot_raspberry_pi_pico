...
# adding screen buffer so position won't flicker
...
def update_cords():
    ...
    # going from -2.8 to 2.8 - left to right
    x_axis = ((ADC2.read_u16() / 65535) - 0.5) * 5.5
    
    # going from -1.4 to 1.4 - up to down
    y_axis = ((ADC1.read_u16() / 65535) - 0.5) * 2.7
    
    # 0 - 100%, 1 - 0% zoom 
    zoom = map(ADC0.read_u16() / 65535, 0, 1, 1, 0)
    ...
    for x in range(WIDTH):
        ...
        for y in range(HEIGHT):
            ...
            if m == maxn:
                ...
                color = int(map(math.sqrt(bright), 0, 1, 0, 255))
...
while True:
    ...
    fb.text("%d%%" % ((1 - zoom) * 100), int(WIDTH/2 + 10), 10, WHITE)
